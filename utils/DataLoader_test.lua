...
DataLoader.create('/home/liuxinyao/bs/code/',256,)
...
function DataLoader.create(data_dir, batch_size, opt, mode)
    ...
    self.batch_data.train = {
        ...
        ['tag'] = torch.ShortTensor(self.batch_size * math.floor(#data.train / self.batch_size)),
    ...
    for i = 1, self.batch_size * self.batch_data.train.nbatches do
        self.batch_data.train.caption[i] = data.train[i]['caption']
    ...
    if opt.gpuid >= 0 then
        self.batch_data.train.caption = self.batch_data.train.question:cuda()
        self.batch_data.train.tag = self.batch_data.train.answer:cuda()
    end

    -- Load val into batches

    print('Loading val fc7 features from /home/liuxinyao/bs/code/data/train_fc7.t7')
...
function DataLoader.json_to_tensor(in_train_c, in_val_c, out_vocab_c, out_tensor)
    ...
    for i in train_c do
        ...
        for token in word_iter(train_c[i]['captions']) do
    ...
    for i in val_c do
        local count = 0
        for token in word_iter(val_c[i]['captions']) do
            if not unordered[token] then
                unordered[token] = 1
            else
                unordered[token] = unordered[token] + 1
    ...
    for i in train_c do
        local caption = {}
        local wl = 0
        for token in word_iter(train_c[i]['caption']) do
            wl = wl + 1
            caption[wl] = c_vocab_mapping[token] or c_vocab_mapping["UNK"]
        end
        data.train[idx] = {
            image_id = train_c[i]['image_id'],
            caption = torch.ShortTensor(max_length):zero(),
            tag = train[i]['image_tag'] == 'yes' and 1 or 0
        }
        for j = 1, wl do
    ...
    for i in val_c do
        local question = {}
        local wl = 0
        for token in word_iter(val_c[i]['caption']) do
            wl = wl + 1
            caption[wl] = c_vocab_mapping[token] or c_vocab_mapping["UNK"]
        end
        data.val[idx] = {
            image_id = val_c[i]['image_id'],
            caption = torch.ShortTensor(max_length):zero(),
            tag = train[i]['image_tag'] == 'yes' and 1 or 0
        }
        for j = 1, wl do
